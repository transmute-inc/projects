...
void dac_write( );
void dac_Test( );
void dac_Sweep(float start, float end, float dv, uint32_t dt, float channel);
...
// select a letter to call the associated routine
	...
	if(spi.cmd=='_') 		{ d_a_Test(); }		//early attempt on MAX11254
	else if(spi.cmd=='b')	{ d_a_bob(); }		//early attempt on MAX11254
	else if(spi.cmd=='a')	{ adc_Test(); }		//cycles dac_test line .4v->1.4v
	...
	else if(spi.cmd=='d')	{ dac_Test( ); }	//cycles all four channels
	else if(spi.cmd=='l')	{ logamp_Test( ); }	//reads voltages on logamps
	else if(spi.cmd=='s')	{  
		dac_Sweep( 0.0, 2.44, 0.1, 0.001, spi.einzel1);}
...
up:		pll_SetIntfreq( megahz );
		usleep(dt);
		megahz = megahz + df;
		if ( megahz < end ) goto up;
	}
	else {
down:	pll_SetIntfreq( megahz );
		usleep(dt);
		megahz = megahz - df;
		if ( megahz > end ) goto down;
	}
...
//  reflected	1		// AIN1P
//  sniffer		2		// AIN2P
		spi.ADC[forward] = spi.ADC[forward] /0.02 - 87; //convert to dBv
		spi.ADC[reflected] = spi.ADC[reflected] /0.02 - 87;
		spi.ADC[sniffer] = spi.ADC[sniffer] /0.02 - 87;	
...
//write dac_test waveform to adc				
ADC:
	if (spi.thread_status == kill ) { return 0; }
	count = count + 1;
	spi.dac_test = .4;
	...
	fprintf( spi.fp, " %d, vin=%.4f  ", count, spi.dac_test);	
	adc_read( );
...
//	spi.ADC[sniffer],spi.ADC[adc_test]);
	...
	printf(" power out = %.4f\r", spi.ADC[forward]);
	goto ADC;
...
void dac_init( )
{
	...
	spi.cathode = 0;			// init v=0 and default gain
...
//  CJ[]     3 2 1 0	
	...
	v[0] = spi.cathode * spi.cathode_gain;
	v[1] = spi.einzel1 * spi.einzel1_gain;
	v[2] = spi.einzel2 * spi.einzel2_gain;
	v[3] = spi.dac_test * spi.dac_test_gain;
...
void dac_Test( )
{
	dac_init( );
DAC:
	if (spi.thread_status == kill ) {	//reset voltage = 0 and bail
		spi.cathode = 0;
		spi.einzel1 = 0;
		spi.einzel2 = 0;
		spi.dac_test = 0;
 ...
 * such as spi.cathode, spi.einzel1, spi.einzel2, or spi.dac_test.
...
down:	dac_write( );
		usleep(dt);
		channel = channel + dv;
		if ( channel < end ) goto down;
	}
	else {
up:		dac_write( );
		usleep(dt);
		channel = channel - dv;
		if ( channel > end ) goto up;
	}
