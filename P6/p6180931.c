...
///////////////////////////////////////////
/*
 * How to test a new board
 * 1. Check voltage and noise on 3.3va, 3.3v, 5va, +12, & 5v (valves)
 * 2. sudo ./p6 
 * 		select a(H2_IN), s(H2_OUT), d(VAC + VAC_PUMP), f(all off)
 * 3. p6  w(start spi thread)
 * 
 * 
 * 
 * 
...
void dac_adc_Test( )  {
	...
	while (spi.thread_status != kill && count < 10) {
		count = count + 1;
		spi.DAC[cathode] = 0.0;
		spi.DAC[einzel1] = 0.0;
		spi.DAC[einzel2] = 0.0;
