 ...
 */
	...
	gage_count = 0;
	while (spi.task_status != kill && gage_count <50) { 

		if( gage_count == 29 ) mass_flow(1,Vac_pump);	// close the vacuum valves
		if( gage_count == 30 ) mass_flow(3,H2_in);		// puff of H2
		if( gage_count == 35 ) {
			gpioWrite(Vac_pump, 1);	// suck out the H2 until next puff 
			gpioWrite(Vac, 1);
		}		
/*		if( gage_count == 69 ) mass_flow(1,Vac_pump);	// close the vacuum valves
		if( gage_count == 70 ) mass_flow(3,H2_in);		// puff of H2
		if( gage_count == 75 ) {
			gpioWrite(Vac_pump, 1);	// suck out the H2 until the end
			gpioWrite(Vac, 1);
		}
*/	
...
//		if( gage_count ==  5 ) mass_flow(3,H2_in);							// puff of H2
		...
		if( gage_count == 30 ) mass_flow(3,H2_in);							// puff of H2
		if( gage_count == 35 ) {spi.DAC[e1]=Vcathode; dac_write( );} 		// pulse grid
		if( gage_count == 37 ) {spi.DAC[e1]= 0; dac_write( );}	
		if( gage_count == 37 ) {spi.DAC[cathode]=Vcathode; dac_write( );} 	// pulse cathode
		if( gage_count == 45 ) {spi.DAC[cathode]= 0; dac_write( );}
...
// grid is actually hooked to e1
	...
	while (count <= 10 ) {
		...
		usleep(200000) ;
		adc_read( );
		fprintf( I_fp, " %.6f %.8f\n", spi.DAC[e1], spi.ADC[ammeter]*1000 );	// diff_amp voltage is divided by r_sense in adc_read
		count++ ;
	}
	spi.DAC[cathode] = 0.0;		//turn off power supplies
	spi.DAC[e1] 	 = 0.0;
	...
	gnuplot_cmd(h1, "set y2tics nomirror tc 'blue' ") ;
   	gnuplot_cmd(h1, "set key left top") ;
	gnuplot_cmd(h1, "set autoscale y2 ") ;
	gnuplot_cmd(h1, "plot './I.dat' using 1:2 with linespoints title 'current' axes x1y2 lc 'blue' ") ;
...
int   ammeter_test( )
{
	...
	gnuplot_cmd(h1, "set y2label 'I mA' tc 'blue' ") ;
	gnuplot_cmd(h1, "set y2tics nomirror tc 'blue' ") ;
	gnuplot_cmd(h1, "set autoscale y2 ") ;
	gnuplot_cmd(h1, "set grid x y ") ;
   	gnuplot_cmd(h1, "set key left top") ;
	
	gnuplot_cmd(h1, "plot './I.dat' using 1:2 with linespoints title 'ammeter' axes x1y2 lc 'blue'  ") ;
	gnuplot_close(h1) ;
	printf("done\n");
