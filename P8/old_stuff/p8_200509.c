...
int   proton_infusion( )
{
	...
	float tictoc, local_max, i_now, trigger, local_max_last, global_max;
	char plot_cmd[96], plotfilename[32];
...
//	dac_write( );
	...
	while (spi.task_status != kill && gage_count < end ) { 
		...
		if( gage_count == next_off ) {
			gpioWrite (H2_in, 0) ;	
			gpioWrite (H2_out, 0) ;
			next_on_set = gage_count + 20;
		}

		serial_gage_read( );
		adc_read( );
		i_now = spi.ADC[ysi] *10;
		clock_t toc = clock();
		tictoc = (double)(toc - tic) / CLOCKS_PER_SEC;
		fprintf( I_fp, " %.4f %.1f %.4f\n", tictoc, vgage.gage, i_now);	
		usleep(1000000) ;
		gage_count++;		
			
		if( i_now > local_max) {
			local_max_last = local_max;
			local_max = i_now;  // i is rising
			if( local_max > global_max ) global_max = local_max;  //promote local max to global max
		}else {
			if(gage_count > next_on_set) {
				if(i_now < local_max * 0.9 ){	  // i is falling
					if( i_now < trigger ) {
						next_on = gage_count + 10; // i is really falling... just wait a little
						next_off = gage_count; // i is really falling!
					} else {
						trigger = i_now;
						next_on = gage_count + 1;	// i is higher than last trigger
					}
				}
			}		
		}
	...
	gnuplot_resetplot(h1) ;
	gnuplot_setstyle(h1, "lines") ;
    sprintf(plot_cmd,"set title 'Proton Infusion max=%.3fmS %s",global_max, plotfilename);
	gnuplot_cmd(h1, plot_cmd) ;
	gnuplot_cmd(h1, "set xlabel 'Time sec' ") ;
	gnuplot_cmd(h1, "set ylabel 'H2 Pressure - Torr' ") ;
