...
int   proton_current( )
{
	int32_t  gage_count, current_count ;
...
// amp_scale was calculated in dac_Sweep
	...
	spi.DAC[cathode] = -0.5;	// ammeter is hooked to cathode
	dac_write( );
	...
	while (spi.task_status != kill && gage_count <40) { 
		...
		while ( current_count < 1 ) {
			...
			fprintf( spi.fp, "cathode=%.4f  v=%.4f  i=%.4e\n", 
				  spi.DAC[cathode], spi.ADC[ammeter], i_measured );	
			clock_t toc = clock();
...
// amp_scale was calculated in dac_Sweep
	...
	count = 0;
	while (count <= 20 ) {
		spi.DAC[cathode] = (float)(count)/10.0;	// ammeter is hooked to cathode
		dac_write( );
		adc_read( );
	...
	gnuplot_resetplot(h1) ;
	gnuplot_setstyle(h1, "lines") ;
    sprintf(plot_cmd,"set title ' ammeter vs. cathode  - Iplot%s.png", date_time);
	gnuplot_cmd(h1, plot_cmd) ;
