...
int main(void)
{
	...
	do 
	{
		...
		if (x == 'a')		// puff of H2
		{
			digitalWrite (H2_in, HIGH) ;delay (500) ;
			digitalWrite (H2_in,  LOW) ;delay (10) ;
			digitalWrite (H2_out,HIGH) ;delay (40) ;
			digitalWrite (H2_out, LOW) ;
		}
		if (x == 's')		// suck of vac
		{
			digitalWrite (Vac_pump, HIGH) ;delay (500) ;
			digitalWrite (Vac_pump,  LOW) ;delay (10) ;
			digitalWrite (Vac,HIGH) ;delay (40) ;
...
void* set_att( )
{
	...
	i = abs(spi.attenuation)*4;	// attenuation in quarter dB steps
	N=i;
...
//		printf("%0x in hexadecimal is %4s\n", reg[0], buff);	
...
*/
		...
		buff[1] = 0x08;		//SEQ  data
			digitalWrite(CS_ADC, 0); //select adc
			wiringPiSPIDataRW(SPI_CHANNEL, buff, 2);
			digitalWrite(CS_ADC, 1); 
...
/*Convert					B    9
 * convert code				1011
 * sample rate=1000sps		    1001
*/
		buff[0] = 0xb9;		// Convert at 1000sps
			digitalWrite(CS_ADC, 0);
			wiringPiSPIDataRW(SPI_CHANNEL, buff, 1);
			digitalWrite(CS_ADC, 1); 
	...
	if( spi.adc_flag == step ) {
		...
		for ( n=0; n<6; n++) {
			...
			digitalWrite(CS_ADC, 0);  	// Low : CS Active
			wiringPiSPIDataRW(SPI_CHANNEL, buff, 4);
			digitalWrite(CS_ADC, 1);
/*			adcOut = 0;
...
void* write_dac( )
{
	...
	if( spi.dac_flag == init ) {
		...
		buff[1] = 0x00;
		buff[2] = 0x00;
		digitalWrite(CS_DAC, 0);  	// Low : CS Active
...
//		buff[1] = (I[n] & 0xff00) >> 8;	// then most significant byte
//		buff[2] = I[n] & 0x00ff;		// then least significant byte
...
void* vac_thread( void* vv )
{
	...
	int h2_in_delay=10, h2_out_delay=5, pump_delay=100, vac_delay=40;
	int dwell_time=800, count=0;
	FILE *fp;
...
Measure: 
	...
	if( v->gage > hi) 
	{									//too high, pulse vac
		vac_delay = 500 * (v->gage-hi) / hi;
		if( vac_delay > 2 ) 
		{
			digitalWrite (Vac_pump, HIGH) ;delay (pump_delay) ;
			digitalWrite (Vac_pump,  LOW) ;delay (10) ;
			digitalWrite (Vac,HIGH) ;delay (vac_delay) ;
			digitalWrite (Vac, LOW) ;
		}	
		goto Measure;
	}
	...
	if( v->gage < low) 
	{									//too low, pulse H2
		h2_out_delay = 20 * (low - v->gage) / low;
		if( h2_out_delay > 2 )
		{
			digitalWrite (H2_in, HIGH) ;delay (h2_in_delay) ;
			digitalWrite (H2_in,  LOW) ;delay (10) ;
			digitalWrite (H2_out,HIGH) ;delay (h2_out_delay) ;
			digitalWrite (H2_out, LOW) ;
		}
		goto Measure;
	}
