...
union equivs { uint32_t J; uint8_t CJ[4]; } eq;
	...
	float_t  r_sense, VchanP, VchanN, temp_exp, temp1, temp2;
...
//	printf(" board rev2 or 6? ");	scanf(" %d", &board_rev);
	board_rev = 6;
	printf(" p9_211215_1430 pellet number? ");		scanf(" %d", &pelletnumber);
	printf(" channel? ");			scanf(" %d", &channelnumber);
	printf(" positive channel voltage - ");	scanf(" %f", &VchanP);
	printf(" negative channel voltage - ");	scanf(" %f", &VchanN);
	printf("pellet = %d  channel%d = %f , %f \n",pelletnumber, channelnumber, VchanN, VchanP);
...
// LTC2664  DAC  50MHz rev6 4x16bit
// MCP23S08 GPIO 10MHz rev6 8 GPIO expander
		spi.adc_fd   = spiOpen(0,  500000, 256);	//ce0   18   12
		spi.dac_fd   = spiOpen(1,  500000, 256);	//ce1   17   11
		spi.range_fd = spiOpen(2,  500000, 256);	//ce2   16   33	
	...
	adc_init( );		// will be reinitiallized in spi_thread
	range_init();
...
osc:		gpioWrite (Sync_pin, 1);
		...
		if (main_cmd == 'v')		// change  voltage
		{
			...
			printf("pellet = %d  channel%d = %.1f , %.1f  R = %.2e \n",
				pelletnumber, channelnumber, VchanN, VchanP, spi.range_resistor[channelnumber] [rangenumber] );
		}			
		if (main_cmd == 'r')		// read channel voltage voltage
		{
			printf(" channel? ");			scanf(" %d", &channelnumber);
			printf( "am%d=%.3e", channelnumber, adc_read(channelnumber) );
		}
 ...
 *  Run this test (wr)
 *  the resistance will cycle 470, 47k,4.7M every 2 sec
*/
	int32_t delay = 200000;
	int32_t count = 0;	
//	char buff[4];	
...
*/
	...
	spi.dac[ch0] 				= 0;
	spi.dac_volts_offset[ch0] 	= 20 * (spi.dac_vref -.00);
...
//		                      channel, buff[0],buff[1],buff[2] );
	...
	fprintf( spi.fp, " dac ->%s = %.4f  dig=%x\n",  spi.dac_names[channel], spi.dac[channel], v_dig ) ;
...
int   sweep_pressure( )
{
	...
	dac_write(cathode, VchanP );	// ammeter is hooked to cathode
 ...
 */
    ...
    sprintf(plot_cmd,"set title 'Proton Current at %.1fV %s",VchanP, plotfilename);
	gnuplot_cmd(h1, plot_cmd) ;
...
//	spi.dac[cathode] = 0.0;		//turn off power supplies
	spi.dac[cathode] = VchanP;	//turn on power supplies
	Vgrid = 48;
...
//		if( gage_count == 35 ) {dac_write(cathode, VchanP);} 	// pulse cathode
...
// grid is actually hooked to e1
	...
	FILE *I_fp;			// open plot file
	I_fp = fopen( "I.dat", "w+" );
	Vx100 = fabs(VchanP *100);
	dac_write(anode, 0.0);
	dac_write(cathode, VchanP );
	
	count = 0;
	while (count <= 10 ) {
		v_set = VchanP * (float)(count)/10.0;	// ammeter is hooked to cathode
		dac_write(e1, v_set );
		usleep(200000) ;
...
int   sweep_channel( int32_t channel)
{
	...
	char plot_cmd[80], plotfilename[32];
	float_t i_sense, V_set, V_diff;

	
	FILE *I_fp;			// open plot file
	I_fp = fopen( "I.dat", "w+" );
	V_diff = (VchanP - VchanN);
	Vx100 =  V_diff* 100;
	...
	steps = 50;
	while (count <= steps ) {
		V_set = VchanN + V_diff * (float)(count)/steps;	// each channel has an ammeter
...
int   opamp_test()
{
	...
	Vset = 50;
	dac_write(cathode, VchanP);	//turn on power supplies
...
void adc_init( )
{
	...
	strcpy(spi.adc_names[6],"Rext1_n");
	strcpy(spi.adc_names[7],"dac_n");
...
//		usleep(1000); 						// sleep does not seem to make any difference
	...
	if(channel <=3 ) spi.adc[channel] = spi.adc_raw[channel] * 0.5 /spi.range_r_value[channel];
...
//	read_chan = channel;	// read the current across Rsense
	...
	delay = 50000;				//5000000 to measure with meter

	count = 0;
	while (spi.task_status != kill && count < 5) {
//		count = count + 1;		// comment out to run continously

		gpioWrite(Sync_pin, 1);
		dac_write(channel, VchanP);	usleep(delay);
		i_read = adc_read(read_chan);
		i_read = spi.adc_raw[read_chan];
		if(count==0) fprintf( spi.fp, "dac_test%d  v_set=%.1f\ti_read=%.4e \n", count, spi.dac[channel], i_read );
		...
		gpioWrite(Sync_pin, 0);
		dac_write(channel, VchanN);	usleep(delay);
		i_read = adc_read(read_chan);
		i_read = spi.adc_raw[read_chan];
		if(count==0) fprintf( spi.fp, "dac_test%d  v_set=%.1f\ti_read=%.4e \n", count, spi.dac[channel], i_read );
		...
		i_read = spi.adc_raw[read_chan];
		if(count==0) fprintf( spi.fp, "dac_test%d  v_set=%.1f\ti_read=%.4e \n", count, spi.dac[channel], i_read );
		if(count==0) fprintf( spi.fp, "\n" );
	}
	
	gpioWrite(Sync_pin, 0);		//clean up on exit
	dac_write(channel, 0);
...
int help_s_menu() {
	printf(
	"\n"
	"wg = sweep_grid - plots grid vs cathode ammeter @ VchanP\n"
	"wi = sweep_pressure - plots pressure vs ammeter @ VchanP\n"
