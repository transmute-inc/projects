...
#define  am2		2		// AIN2P	cat
#define  am3		3		// AIN3P	e1
#define  gage1		4		// AIN4P	(H2)
#define  gage2		5		// AIN5P	(vac)
...
void dac_adc_test(int32_t channel);
int  dac_spare_test( );
...
int main(void)
{
	...
	printf(" pellet number? ");		scanf(" %d", &pelletnumber);
...
*/		spi.adc_fd   = spiOpen(0,  7000000, 256);	//ce0   18   12
		spi.dac_fd   = spiOpen(1, 10000000, 256);	//ce1   17   11
		spi.range_fd = spiOpen(2,  1000000, 256);	//ce2   16   33	
	}
...
//	adc_init( 2 );	// mode 1 code does not work yet, use mode 2
	...
	if     (spi.cmd=='a'){spi.cmd=0;dac_adc_test(channelnumber); }	//cycles dac_test line 0v->2.44v
	...
	else if(spi.cmd=='b'){spi.cmd=0;sweep_channel(channelnumber); }	
...
// set up channel names
	...
	spi.dac_channel_rev6[5] = 2;	// map e1,e2, e3top, e3bot, & dacspare -> 2
	spi.dac_channel_rev6[6] = 2;
	spi.dac_channel_rev6[7] = 2;
...
*/
	if( board_rev == 2) {
		spi.dac_bits = 65535;	// 2^16-1bit, ad5668 0->+vref
		spi.dac_vref = 2.534;

		spi.dac[anode] 			= 0;
		spi.dac_volts_offset[anode] 	= 0;
		spi.dac_bits_per_volt[anode] 	= spi.dac_bits  / 50;
	
		spi.dac[grid] 			= 0;
		spi.dac_volts_offset[grid] 	= 0;
		spi.dac_bits_per_volt[grid] 		= spi.dac_bits  / 50;
	
		spi.dac[e1] 			= 0;
		spi.dac_volts_offset[e1] 		= 0;
		spi.dac_bits_per_volt[e1] 		= spi.dac_bits  / 50;
	
		spi.dac[e2] 			= 0;
		spi.dac_volts_offset[e2] 		= 0;
		spi.dac_bits_per_volt[e2] 		= spi.dac_bits  / 50;
		
		spi.dac[e3top] 			= 0;
		spi.dac_volts_offset[e3top] 	= 0;
		spi.dac_bits_per_volt[e3top] 	= spi.dac_bits  / 50;
		
		spi.dac[e3bot] 			= 0;
		spi.dac_volts_offset[e3bot] 	= 0;
		spi.dac_bits_per_volt[e3bot] 	= spi.dac_bits  / 50;
		
		spi.dac[cathode] 		= 0;
		spi.dac_volts_offset[cathode] = .0421;
		spi.dac_bits_per_volt[cathode] 	= spi.dac_bits  / 49.6;
		
		spi.dac[dac_spare] 		= 0;
		spi.dac_volts_offset[dac_spare] = 0;
		spi.dac_bits_per_volt[dac_spare] = spi.dac_bits  / spi.dac_vref;	
	}
	...
	if( board_rev == 6) {
		...
		spi.dac[ch0] 				= 0;
		spi.dac_volts_offset[ch0] 	= 50 -0.004;
		spi.dac_bits_per_volt[ch0] 	= spi.dac_bits  / 100;
	
		spi.dac[ch1] 				= 0;
		spi.dac_volts_offset[ch1] 	= 50 +0.004;
		spi.dac_bits_per_volt[ch1] 	= spi.dac_bits  / 100;
		
		spi.dac[ch2] 				= 0;
		spi.dac_volts_offset[ch2] 	= 50 +0.007;
		spi.dac_bits_per_volt[ch2] 	= spi.dac_bits  / 100;
		
		spi.dac[ch3] 				= 0;
		spi.dac_volts_offset[ch3] 	= 50 +0.002;
 ...
 * 0 = 0v, 65535 = +2.5v  (anode, grid 0,+50v), rest 0,-50v
 * 
...
*/
	...
	if(board_rev == 6) {
		temp = channel; 		//spi.dac_channel_rev6[channel];
		v_dig = ( value + spi.dac_volts_offset[temp] ) * spi.dac_bits_per_volt[temp];
		if( v_dig > spi.dac_bits) v_dig = spi.dac_bits;	
		if( v_dig < 0.0         ) v_dig = 0.0;
		
		eq.J = spi.dac_cmd[temp] + v_dig;
		buff[0] = eq.CJ[2];		// uses same command and address as ad5668
		buff[1] = eq.CJ[1];
		buff[2] = eq.CJ[0];

/*		buff[0] = 0x30 + temp;  //this should make the same word
		buff[1] = v_dig >>8;
		buff[2] = v_dig & 0x00ff;
*/		
...
/*	eq.J = eq.J <<4;
	...
	spiWrite(spi.dac_fd, buff, 4); 
*/	
//	fprintf( spi.fp, " dac ->%s = %.4f\n",  spi.dac_names[temp], spi.dac[temp] ) ;
...
int   sweep_channel( int32_t channel)
{
	...
	I_fp = fopen( "I.dat", "w+" );
	Vx100 = Vchannel *100;
	i_sense = adc_read(channel);						//clear out previous read
	
	count = 0;
	while (count <= 100 ) {
		v_set = Vchannel * (float)(count)/100.0;	// each channel has an ammeter
    ...
    sprintf(plot_cmd,"set output '%s'",plotfilename);
	gnuplot_cmd(h1, plot_cmd) ;
    sprintf(plot_cmd,"set title ' ammeter vs. channel%d  - %s", channel, plotfilename);
	gnuplot_cmd(h1, plot_cmd) ;
...
int   dac_spare_test( )
{
	...
	count = 0;
	while (count <= 10 ) {
		Vset = Vchannel * (float)(count)/10.0;	
...
void adc_init( uint32_t mode )
{
	...
	if( board_rev ==2) {
		...
		strcpy(spi.adc_names[0],"gage_Vac");
		strcpy(spi.adc_names[1],"ammeter");
		strcpy(spi.adc_names[2],"dead");
		strcpy(spi.adc_names[3],"adc_3");
		strcpy(spi.adc_names[4],"gage_H2");
		strcpy(spi.adc_names[5],"adc_spar");
...
*/
	...
	if( board_rev ==6) {
		...
		spi.adc_channel_rev6[0] = 5;	//gage_Vac -> gage2
		spi.adc_channel_rev6[1] = 3;	//ammeter -> cathode
		spi.adc_channel_rev6[2] = 1;	//grid
		spi.adc_channel_rev6[3] = 3;	//adc_3 -> cathode
		spi.adc_channel_rev6[4] = 4;	//gage_H2 -> gage1 	
		spi.adc_channel_rev6[5] = 3;	//adc_spare -> cathode
 ...
 * sample rate=64000sps		    1110  */
		...
		eq.CJ[2] = buff_rx[1];
		eq.CJ[3] = buff_rx[0];
//		fprintf( spi.fp, "adc buff[0-3]  %02x, %02x, %02x, %02x\n", 
//		                         buff[0],buff[1],buff[2],buff[3] );
		spi.adc_raw[channel] = (float) eq.J / spi.adc_bits * spi.adc_vref;
	...
	if( board_rev ==6) {
//		channel = spi.adc_channel_rev6[channel];	//map rev2 channels into rev6
		eq.J = 0;
		buff[0] = (0x30 + channel)<<2;		//command for Man_Ch selection
		buff[1] = 0x00;						//pad 16bit command
		spiXfer(spi.adc_fd, buff, buff_rx, 5);
		spiXfer(spi.adc_fd, buff, buff_rx, 5);	//reports second read
		eq.CJ[0] = buff_rx[4];
		eq.CJ[1] = buff_rx[3];
		eq.CJ[2] = buff_rx[2];
		eq.J = eq.J >> 6;					//shift 18bits to lsb
//		fprintf( spi.fp, " adc%d %04x, buff_rx  %02x, %02x, %02x, %02x, %0x\n", 
//		                    channel, eq.J, buff_rx[0],buff_rx[1],buff_rx[2],buff_rx[3],buff_rx[4] );
                
		spi.adc_raw[channel] = (float) (eq.J ) / (spi.adc_bits) * spi.adc_vref*2 - spi.adc_vref;
		
		if(channel <=3 ) spi.adc[channel] = spi.adc_raw[channel] * 0.7 /spi.range_r_value[channel];
		if(channel == gage1) spi.adc[gage1] = pow( 10, spi.adc_raw[gage1]/0.24485 - 6  );    //ratio of voltage divider
		if(channel == gage2) spi.adc[gage2] = pow( 10, spi.adc_raw[gage2]/0.24485 - 6  );    //ratio of voltage divider
	}
...
void dac_adc_test(int32_t channel)  {
	int32_t count = 0;
	int32_t delay = 5000000;	//5000000 to measure with meter
	
	while (spi.task_status != kill && count < 5) {
		count = count + 1;

		range_set(channel,range_50ua);	//range_5ma, range_50ua,range_500pa
		
		gpioWrite(Sync_pin, 1);
		dac_write(channel, Vchannel);
		usleep(delay);
		fprintf( spi.fp, "dac_test%d  v_set=%.1f\ti_read=%.2e \n", count, spi.dac[channel], adc_read(channel) );
		usleep(10);
		fprintf( spi.fp, "dac_test%d  v_set=%.1f\ti_read=%.2e \n", count, spi.dac[channel], adc_read(channel) );		
