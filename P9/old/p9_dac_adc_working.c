...
*/		spi.adc_fd   = spiOpen(0, 1000000, 256);	//ce0   18   12
		spi.dac_fd   = spiOpen(1, 1000000, 256);	//ce1   17   11
...
//		                         buff[0],buff[1],buff[2],buff[3] );
	...
	if( board_rev ==6) {
		...
		eq.CJ[1] = buff_rx[3];
		eq.CJ[2] = buff_rx[2];
//		eq.CJ[3] = buff_rx[1];				//msB should be zero
...
//		                    channel, eq.J, buff_rx[0],buff_rx[1],buff_rx[2],buff_rx[3],buff_rx[4] );
		...
		spi.adc_raw[channel] = (float) (v_dig ) / (spi.adc_bits) * spi.adc_vref*2 - spi.adc_vref +0.0005;
	}
...
void dac_adc_Test( )  {
	...
	while (spi.task_status != kill && count < 3) {
		count = count + 1;
		gpioWrite(Sync_pin, 1);

		range_set(anode,0);			//rev6-0
		
		dac_write(anode, 50);
		usleep(2);
		fprintf( spi.fp, "\ndac_test%d  v_set=%.4f  v_read=%.4f \n", count, spi.dac[anode], adc_read(gage_H2) );
		
		dac_write(anode, -50);	
		usleep(2);
		fprintf( spi.fp, "dac_test%d  v_set=%.4f  v_read=%.4f \n", count, spi.dac[anode], adc_read(gage_H2) );

/*		range_set(grid,0);			//rev6-1
		dac_write(grid, 50);
		range_set(e1,0);			//rev6-2
		dac_write(e1, -50);
		range_set(cathode,0);		//rev6-3
