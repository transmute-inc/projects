...
//	adc_init( 2 );	// mode 1 code does not work yet, use mode 2
	...
	if     (spi.cmd=='a'){spi.cmd=0;dac_adc_test(anode); }	//cycles dac_test line 0v->2.44v
	else if(spi.cmd=='v'){spi.cmd=0;gpio_Test( ); }		//valve test
	else if(spi.cmd=='r'){spi.cmd=0;range_test( ch0 ); }	//
...
*/
	...
	fprintf( spi.fp, " range%d  relay=%02x  reg=%02x\n", channel, max_current, spi.range_out);
...
*/
	...
	if( board_rev == 6) {
		...
		spi.dac[anode] 					= 0;
		spi.dac_volts_offset[anode] 	= 50;
		spi.dac_bits_per_volt[anode] 	= spi.dac_bits  / 100;
	
		spi.dac[grid] 					= 0;
		spi.dac_volts_offset[grid] 		= 50;
		spi.dac_bits_per_volt[grid] 	= spi.dac_bits  / 100;
		
		spi.dac[e1] 					= 0;
		spi.dac_volts_offset[e1] 		= 50;
		spi.dac_bits_per_volt[e1] 		= spi.dac_bits  / 100;
		
		spi.dac[cathode] 				= 0;
		spi.dac_volts_offset[cathode] 	= 50 - .0421;
		spi.dac_bits_per_volt[cathode] 	= spi.dac_bits  / 99.6;
...
*/
	...
	if(board_rev == 6) {
		temp = spi.dac_channel_rev6[channel];
...
//		                    channel, eq.J, buff_rx[0],buff_rx[1],buff_rx[2],buff_rx[3],buff_rx[4] );
		...
		spi.adc_raw[channel] = (float) (eq.J ) / (spi.adc_bits) * spi.adc_vref*2 - spi.adc_vref +0.0006;
	}
	
//	fprintf( spi.fp, " adc<- %s = %.4f\n",  spi.adc_names[channel], spi.adc_raw[channel]);
...
//	if(channel == gage_H2) spi.adc[gage_H2] = pow( 10, spi.adc_raw[gage_H2]/0.24485 - 6  );    //ratio of voltage divider
	if(channel == gage_H2) spi.adc[gage_H2] = spi.adc_raw[gage_H2];
...
//	if(channel == gage_Vac) spi.adc[gage_Vac]  = pow( 10, spi.adc_raw[gage_Vac]/0.24605 - 6  );    //ratio of voltage divider
	if(channel == gage_Vac) spi.adc[gage_H2] = spi.adc_raw[gage_Vac];
...
void dac_adc_test(int32_t channel)  {
	int32_t count = 0;
	int32_t delay = 2;	//5000000 to measure with meter
...
//		range_set(channel,0);	
		...
		gpioWrite(Sync_pin, 1);
		dac_write(channel, 50);
		usleep(delay);
		fprintf( spi.fp, "dac_test%d  v_set=%.2f\tv_read=%.4f \n", count, spi.dac[channel], adc_read(gage_H2) );
