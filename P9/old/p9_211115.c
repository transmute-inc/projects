...
*/		spi.adc_fd   = spiOpen(0,  2000000, 256);	//ce0   18   12
		spi.dac_fd   = spiOpen(1,  2000000, 256);	//ce1   17   11
		spi.range_fd = spiOpen(2,  2000000, 256);	//ce2   16   33	
	}
...
osc:		gpioWrite (Sync_pin, 1);
		...
		if (main_cmd == 'a') {		// puff of H2
			...
			printf( "am%d=%.3e  H2=%.2e", channelnumber, adc_read(channelnumber)*1000, adc_read(gage1) );
			fflush(stdout);
		}
...
*/
	...
	if( board_rev == 6) {
		...
		spi.dac[ch0] 				= 0;
		spi.dac_volts_offset[ch0] 	= 50 -.002;
...
//		fprintf( I_fp, " %.6f %.8f\n", v_set, i_sense*1000 );	// diff_amp voltage is divided by r_sense in adc_read
		v_set = v_set/20;		// voltage on dac
		adc_read(gage1);
		fprintf( I_fp, " %.6f %.8f\n", v_set, spi.adc_raw[gage1] );	// test loop dac0->adc(gage1)
		count++ ;
	}
...
//		usleep(1000); 						// sleep does not seem to make any difference
	...
	if(channel == gage1) spi.adc[gage1] = pow( 10, spi.adc_raw[gage1]*4.0888 - 6  );    //ratio of voltage divider
	if(channel == gage2) spi.adc[gage2] = pow( 10, spi.adc_raw[gage2]*4.0888 - 6  );    //ratio of voltage divider
